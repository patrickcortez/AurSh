...
export const PlaylistHeader: React.FC<PlaylistHeaderProps> = ({ playlist, refreshUserData }) => {
  ...
  return (
    <div 
      ...
      style={{ 
        position: 'relative',
        background: playlist.bannerArt 
          ? `linear-gradient(to bottom, rgba(0,0,0,0.2) 0%, rgba(18,18,18,1) 100%), url(${playlist.bannerArt}) center/cover no-repeat`
          : 'linear-gradient(to bottom, #450af5, #121212)',
        display: 'flex', 
        alignItems: 'flex-end', 
        padding: '24px', 
        height: '30vh', 
    ...
    >
      <div style={{ display: 'flex', gap: '24px', alignItems: 'flex-end', zIndex: 1 }}>
        <div 
          style={{ width: '192px', height: '192px', cursor: 'pointer', position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 60px rgba(0,0,0,.5)' }}
